package com.example.mcaadmissionhelp.util;

public class URLs {
    private static final String ROOT_URL = "https://challenge123.000webhostapp.com/";
    //private static final String ROOT_URL = "http://192.168.31.184/gssummit/";


    public static final String URL_REGISTER = ROOT_URL + "register.php";
    public static final String URL_LOGIN = ROOT_URL + "login.php";
    public static final String URL_COLLEGES = ROOT_URL + "getColleges.php";
    public static final String URL_PROFILE = ROOT_URL + "getProfile.php";
    public static final String URL_SEND_MESSAGE = ROOT_URL + "sendMessage.php";
    public static final String URL_GET_MESSAGING_USERS = ROOT_URL + "getMessagingUsers.php";
    public static final String URL_GET_MESSAGES = ROOT_URL + "getMessages.php";
    public static final String URL_UPDATE_TOKEN = ROOT_URL + "updateToken.php";
    public static final String URL_UPLOAD_IMAGE = ROOT_URL + "uploadImage.php";


}
